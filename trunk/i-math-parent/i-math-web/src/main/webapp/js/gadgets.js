...
/**
 * Calls an array of asynchronous functions and calls the continuation function
 * when all are done.
 * 
 * @param {Array}
 *            functions Array of asynchronous functions, each taking one
 *            argument that is the continuation function that handles the result
 *            That is, each function is something like the following:
 *            function(continuation) { // compute result asynchronously
 *            continuation(result); }
 * @param {Function}
 *            continuation Function to call when all results are in. It is pass
 *            an array of all results of all functions
 * @param {Object}
 *            opt_this Optional object used as "this" when calling each function
 */
 ...
 * 
 * @param {Object}
 *            dependencies Object whose properties are set on this container as
 *            dependencies
 */
...
/**
 * User preference store implementation. TODO: Turn this into a real
 * implementation that is production safe
 * 
 * @constructor
 */
...
/**
 * Interface of service provided to gadgets for resizing gadgets, setting title,
 * etc.
 * 
 * @constructor
 */
 ...
 *            editToken
 * @param {String}
 *            name Name of user preference
 * @param {String}
 *            value Value of user preference More names and values may follow
 */
 ...
 * real containers. TODO: Find a better default for this function
 * 
 * @param view
 *            The view name to get the url for
 */
 ...
 * 
 * @param {Array}
 *            gadgetChromeIds Gadget id to chrome id map
 */
 ...
 * @constructor
 * @param {String}
 *            layoutRootId Id of the element that is the parent of all gadgets.
 */
 ...
 * @constructor
 * @param {Object}
 *            params Parameters to set on gadget. Common parameters: "specUrl":
 *            URL to gadget specification "private": Whether gadget spec is
 *            accessible only privately, which means browser can load it but not
 *            gadget server "spec": Gadget Specification in XML "viewParams": a
 *            javascript object containing attribute value pairs for this
 *            gadgets "secureToken": an encoded token that is passed on the URL
 *            hash "hashData": Query-string like data that will be added to the
 *            hash portion of the URL. "specVersion": a hash value used to add a
 *            v= param to allow for better caching "title": the default title to
 *            use for the title bar. "height": height of the gadget "width":
 *            width of the gadget "debug": send debug=1 to the gadget server,
 *            gets us uncompressed javascript
 */
...
gadgets.Gadget.prototype.render = function(chrome) {
	if (chrome) {
		...
		this.getContent( function(content) {
			...
			window.frames[gadget.getIframeId()].location = gadget
					.getIframeUrl();
		});
	}
};
...
gadgets.Gadget.prototype.getContent = function(continuation) {
	gadgets.callAsyncAndJoin( [ this.getTitleBarContent,
			this.getUserPrefsDialogContent, this.getMainContent ], function(
			results) {
 ...
 * 
 * @param {Function}
 *            continuation Function that handles title bar content as the one
 *            and only argument
 */
 ...
 * 
 * @param {Function}
 *            continuation Function that handles user preferences content as the
 ...
 * 
 * @param {Function}
 *            continuation Function that handles gadget content as the one and
 *            only argument
 */
...
/*
 * Gets additional parameters to append to the iframe url Override this method
 * if you need any custom params.
 */
...
gadgets.IfrGadget.prototype.getTitleBarContent = function(continuation) {
	continuation('<div id="'
			...
			+ this.cssClassTitleButton + '">toggle</a></span></div>');
};
...
gadgets.IfrGadget.prototype.getMainContent = function(continuation) {
	...
	continuation('<div class="' + this.cssClassGadgetContent + '"><iframe id="'
			...
			+ this.cssClassGadget + '" src="about:blank'
...
gadgets.IfrGadget.prototype.getIframeUrl = function() {
	return this.serverBase_
			...
			+ this.id
			+ '&nocache='
			+ gadgets.container.nocache_
			+ '&country='
			+ gadgets.container.country_
			+ '&lang='
			+ gadgets.container.language_
			+ '&view='
			+ gadgets.container.view_
			...
			+ '#rpctoken='
			+ this.rpcToken
			+ (this.viewParams ? '&view-params=' + encodeURIComponent(gadgets.json
...
gadgets.IfrGadget.prototype.handleOpenUserPrefsDialog = function() {
	if (this.userPrefsDialogContentLoaded) {
		...
		script.src = 'http://gmodules.com/ig/gadgetsettings?mid=' + this.id
				+ '&output=js' + this.getUserPrefsParams() + '&url='
				+ this.specUrl;
...
gadgets.IfrGadget.prototype.buildUserPrefsDialog = function(content) {
	...
	userPrefsDialog.innerHTML = content
			+ '<div class="'
			+ this.cssClassGadgetUserPrefsDialogActionBar
...
/**
 * Known dependencies: gadgetClass: constructor to create a new gadget instance
 * userPrefStore: instance of a subclass of gadgets.UserPrefStore gadgetService:
 * instance of a subclass of gadgets.GadgetService layoutManager: instance of a
 * subclass of gadgets.LayoutManager
 */
