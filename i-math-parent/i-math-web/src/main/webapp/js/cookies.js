 ...
 * @param {number}
 *            opt_maxAge The max age in seconds (from now). Use -1 to set a
 *            session cookie. If not provided, the default is -1 (i.e. set a
 *            session cookie).
 * @param {string}
 *            opt_path The path of the cookie, or null to not specify a path
 *            attribute (browser will use the full request path). If not
 *            provided, the default is '/' (i.e. path=/).
 * @param {string}
 *            opt_domain The domain of the cookie, or null to not specify a
 *            domain attribute (browser will use the full request host name). If
 *            not provided, the default is null (i.e. let browser use full
 *            request host name).
 */
shindig.cookies.set = function(name, value, opt_maxAge, opt_path, opt_domain) {
	...
	if (opt_maxAge < 0) {
		...
		// it to local time, and if the local time is before Jan 1, 1970, then
		// the
 ...
 * @param {string}
 *            opt_default If not found this is returned instead.
 ...
 *            name The cookie name.
 * @param {string}
 *            opt_path The path of the cookie, or null to expire a cookie set at
 *            the full request path. If not provided, the default is '/' (i.e.
 *            path=/).
 * @param {string}
 *            opt_domain The domain of the cookie, or null to expire a cookie
 *            set at the full request host name. If not provided, the default is
 ...
 * 
 * @param {string}
 *            key The name of the cookie to test for
 * @return {boolean}
 */
...
/**
 * Static constant for the size of cookies. Per the spec, there's a 4K limit to
 * the size of a cookie. To make sure users can't break this limit, we should
 * truncate long cookies at 3950 bytes, to be extra careful with dumb
